    ...
    <div class="w-full lg:w-1/2 px-3">
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
            <div class="p-6 pb-0 mb-0 bg-white border-b-0 rounded-t-2xl">
                <h6>Video Graphics Adapters (VGA)</h6>
            </div>
            <div class="flex-auto px-0 pt-0 pb-2">
                <div class="p-0 overflow-x-auto">
                    <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                        ...
                        <tbody>
                            @forelse($vga as $index => $item)
                                <tr>
                                    <td class="w-[50px] p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                        <span class="text-xs font-semibold leading-tight text-slate-400">{{ $vga->firstItem() + $index }}</span>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="12" class="text-sm text-slate-400 italic text-center">No vga found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                    @if ($vga->hasPages())
                        <nav class="flex items-center justify-center space-x-1 mt-4 text-sm">
                            ...
                            @else
                                <a href="{{ $vga->previousPageUrl() }}" class="px-2 py-0.5 border rounded hover:bg-gray-100 mx-1 transition duration-200 transform hover:scale-105">&lt;</a>
                            @endif

                            {{-- Custom Pagination Elements --}}
                            @php
                                $currentPage = $vga->currentPage();
                                $lastPage = $vga->lastPage();
                            ...
                            @for ($page = $start; $page <= $end; $page++)
                                ...
                                @else
                                    <a href="{{ $vga->url($page) }}" class="px-2 py-0.5 border rounded hover:bg-gray-100 mx-2 transition duration-200 transform hover:scale-105">{{ $page }}</a>
                                @endif
                            @endfor

                            {{-- Next Page Link --}}
                            @if ($vga->hasMorePages())
                                <a href="{{ $vga->nextPageUrl() }}" class="px-2 py-0.5 border rounded hover:bg-gray-100 mx-2 transition duration-200 transform hover:scale-105">&gt;</a>
                            @else
                                <span class="px-2 py-0.5 border rounded text-gray-400 mx-2 transition duration-200 transform">&gt;</span>
                            @endif
                        </nav>
