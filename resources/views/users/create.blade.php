            ...
            <div class="relative flex flex-col min-w-0 mb-6 mt-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded bg-clip-border">
                <div class="p-6 pb-2 mb-0 bg-gradient-to-r from-blue-700 to-yellow-400 border-b-0 border-b-solid rounded border-b-transparent flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <!-- Back Icon -->
                        <a href="{{ route('users.index') }}" class="text-white hover:text-slate-800 text-base transition-transform transform hover:scale-110 mr-4" title="Back">
                            <i class="fas fa-arrow-left mb-2"></i>
                        </a>
                        <h6 class="text-lg font-semibold text-white">User</h6>
                    </div>
                </div>
                        ...
                        <form action="{{ route('users.store') }}" method="POST" class="space-y-4">
                            ...
                            <div>
                                <label for="password" class="block text-sm font-medium text-slate-700">
                                    Password {{ isset($user) ? '(leave blank to keep current)' : '' }} <span class="{{ isset($user) ? '' : 'text-red-500' }}">*</span>
                                </label>
                            ...
                            <div>
                                <label for="password_confirmation" class="block text-sm font-medium text-slate-700">
                                    Confirm Password <span class="{{ isset($user) ? '' : 'text-red-500' }}">*</span>
                                </label>
