...
<body>
    <div class="container">
        ...
        @php
            ...
            $sortedLogs = $transfers->sortBy('created_at');
        
            $firstTransfer = $sortedLogs->first();
            $lastTransfer = $sortedLogs->last();
        
            $startTime = $firstTransfer ? Carbon::parse($firstTransfer->created_at)->format('H:i') : '-';
            $endTime = $lastTransfer ? Carbon::parse($lastTransfer->created_at)->format('H:i') : '-';
        @endphp

        <div class="content">
            <p>
                @if($firstTransfer)
                    Pada Hari {{ Carbon::parse($firstTransfer->created_at)->translatedFormat('l') }} 
                    Tanggal {{ Carbon::parse($firstTransfer->created_at)->format('d') }} 
                    Bulan {{ Carbon::parse($firstTransfer->created_at)->translatedFormat('F') }} 
                    Tahun {{ Carbon::parse($firstTransfer->created_at)->format('Y') }} 
                    dari jam {{ $startTime }} sampai dengan jam {{ $endTime }} telah dilakukan pemindahan aset sebagai berikut:
                @else
                    <p style="text-align: center; font-style: italic; color: #6b7280;">ada data pemindahan aset pada periode ini.</p>
                @endif
            </p>            
